...
class DDPMixSolver(object):
    ...
    def train(self, epoch):
        ...
        for i, (img_tensor, valid_size, targets_tensor, mask_tensor, batch_len) in enumerate(pbar):
            ...
            str_template = "epoch:{:2d}|size:{:3d}|loss:{:6.4f}|cls:{:6.4f}|mask:{:6.4f}|num:{:0>4d}|lr:{:8.6f}"
            if self.local_rank == 0:
        ...
        if self.local_rank == 0:
            final_template = "epoch:{:2d}|loss:{:6.4f}|cls:{:6.4f}|mask:{:6.4f}"
    ...
    def val(self, epoch):
        ...
        for img_tensor, valid_size, targets_tensor, mask_tensor, batch_len in pbar:
            img_tensor = img_tensor.to(self.device)
            targets_tensor = targets_tensor.to(self.device)
            predicts = self.ema.ema(img_tensor, valid_size=valid_size)['predicts']
